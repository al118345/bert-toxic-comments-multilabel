...
BERT_PRETRAINED_PATH = Path(
    './complaints/bert/pretrained-weights/uncased_L-12_H-768_A-12/')
...
class MultiLabelTextProcessor(DataProcessor):
    ...
    def get_train_examples(self):
        ...
        print(len(conexionBase.conectar_al_traintest()))
        ...
        for i in todos:
            if indice < (len(todos) -350):
                if (i[8] == 'positivo'):
                    train = load_directory_data(i[0], [0, 1, 0], train)
                if (i[8] == 'negativo'):
                    train = load_directory_data(i[0], [1, 0, 0], train)
                if (i[8] == 'neutral'):
                    train = load_directory_data(i[0], [0, 0, 1], train)

            else:
                if (i[8] == 'positivo'):
                    test = load_directory_data(i[0], [0, 1, 0], test)
                if (i[8] == 'negativo'):
                    test = load_directory_data(i[0], [1, 0, 0], test)
                if (i[8] == 'neutral'):
                    test = load_directory_data(i[0], [0, 0, 1], test)
            indice = indice + 1
...
def load_directory_data(lista_tweet, polaridad, data):
    data["sentence"].append(lista_tweet)
...
'''
Lo modifico con la ide de que utilice los datos de mi conexión
...
num_train_steps = int(len(train_examples) / args['train_batch_size'] / args[
...
class CyclicLR(object):
    ...
    def get_lr(self):
        ...
        lrs = []
        param_lrs = zip(self.optimizer.param_groups, self.base_lrs,
                        self.max_lrs)
        for param_group, base_lr, max_lr in param_lrs:
            ...
            if self.scale_mode == 'cycle':
                ...
                lr = base_lr + base_height * self.scale_fn(
...
scheduler = CyclicLR(optimizer, base_lr=2e-5, max_lr=5e-5, step_size=2500,
...
model = BertForMultiLabelSequenceClassification.from_pretrained(
    'bert-base-uncased', num_labels=num_labels, state_dict=model_state_dict)
model.to(device)
model
...
def predict(model, path, test_filename='test.csv'):
    ...
    input_data = [
        {'id': input_example.guid, 'comment_text': input_example.text_a} for
        input_example in test_examples]
    ...
    return pd.merge(pd.DataFrame(input_data),
...
result.to_csv(DATA_PATH / 'pol.csv', index=None)
...
for z in pred_sentences:
    ...
    print(z)
textos = []
for textos in pred_sentences:
    textos.append(textos[0])
'''

df = pd.DataFrame(list(result))

writer = pd.ExcelWriter("pol" + '.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name="Analisis")
workbook = writer.book

writer.save()

writer = pd.ExcelWriter("polsent" + '.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name="Analisis")
workbook = writer.book
df = pd.DataFrame(list(pred_sentences))
writer.save()
